...
class S3DBTileLoader extends TileLoader {
    ...
    static {
        ...
        mTilePlane.tags.add(new Tag(S3DBTag.KEY_COLOR, "transparent"));
    }
    ...
    public void process(MapElement element) {
        ...
        boolean isRoof = element.tags.containsKey(S3DBTag.KEY_ROOF);
        //if (isRoof)
        //    log.debug(element.tags.toString());

        int c = 0;
        if (element.tags.containsKey(S3DBTag.KEY_COLOR)) {
            c = S3DBLayer.getColor(element.tags.getValue(S3DBTag.KEY_COLOR), isRoof);
        }

        if (c == 0 && element.tags.containsKey(S3DBTag.KEY_MATERIAL)) {
            c = getMaterialColor(element.tags.getValue(S3DBTag.KEY_MATERIAL), isRoof);
        }

        if (c == 0) {
            String roofShape = element.tags.getValue(S3DBTag.KEY_ROOF_SHAPE);
